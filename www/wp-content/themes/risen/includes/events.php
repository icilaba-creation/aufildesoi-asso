...
function risen_event_post_type() {
	...
	register_post_type(
		...
		array(
			...
			'taxonomies' 		=> array( 'risen_event_category' )
		)
	);
...
function risen_envent_category_taxonomy() {

	register_taxonomy(
		'risen_envent_category',
		'risen_event',
		array(
			'labels' => array(
				'name' 						=> sprintf( _x( '%s Categories', 'taxonomy general name', 'risen' ), risen_option( 'event_word_singular' ) ),
				'singular_name'				=> sprintf( _x( '%s Category', 'taxonomy singular name', 'risen' ), risen_option( 'event_word_singular' ) ),
				'search_items' 				=> _x( 'Search Categories', 'event', 'risen' ),
				'popular_items' 			=> _x( 'Popular Categories', 'event', 'risen' ),
				'all_items' 				=> _x( 'All Categories', 'event', 'risen' ),
				'parent_item' 				=> null,
				'parent_item_colon' 		=> null,
				'edit_item' 				=> _x( 'Edit Category', 'event', 'risen' ), 
				'update_item' 				=> _x( 'Update Category', 'event', 'risen' ),
				'add_new_item' 				=> _x( 'Add Category', 'event', 'risen' ),
				'new_item_name' 			=> _x( 'New Category', 'event', 'risen' ),
				'separate_items_with_commas' => _x( 'Separate categories with commas', 'event', 'risen' ),
				'add_or_remove_items' 		=> _x( 'Add or remove categories', 'event', 'risen' ),
				'choose_from_most_used' 	=> _x( 'Choose from the most used categories', 'event', 'risen' ),
				'menu_name' 				=> _x( 'Categories', 'event menu name', 'risen' )
			),
...
function risen_event_meta_boxes_add() {
	...
	// Type
	add_meta_box(
		'risen_event_type',						// Unique meta box ID
		__( 'Event Type', 'risen' ),					// Title of meta box
		'risen_event_type_meta_box_html',		// Callback function to output HTML
...
function risen_event_type_meta_box_html( $object, $box ) {
	...
	<p>
		<div class="risen-meta-value">
			<select name="<?php echo $meta_key; ?>" id="<?php echo $meta_key; ?>">
				<?php echo risen_event_type_options( $meta_value ); ?>
			</select>
			<p class="description">
				<?php _e( 'You can set en event as a conference, an atelier, a stage or a course.', 'risen' ); ?>
			</p>
		</div>
	</p>
...
if ( ! function_exists( 'risen_event_type_options' ) ) {

	function risen_event_type_options( $selected = false ) {

		$types = risen_event_types();
...
if ( ! function_exists( 'risen_event_types' ) ) {

	function risen_event_types() {

		$types = array(
			'conference'	=> _x( 'Conference', 'map', 'risen' ),
			'atelier'	=> _x( 'Atelier', 'map', 'risen' ),
			'stage'	=> _x( 'Stage', 'map', 'risen' ),
			'formationwxc'	=> _x( 'Formation', 'map', 'risen' )
		);
		
		return apply_filters( 'risen_event_types', $types );
...
function risen_event_location_meta_box_html( $object, $box ) {
	...
	<p>
		...
		<div class="risen-meta-value risen-meta-medium">
			<input type="text" name="<?php echo $meta_key; ?>" id="<?php echo $meta_key; ?>" value="<?php echo esc_attr( get_post_meta( $object->ID, $meta_key, true ) ); ?>" size="30" />
			<p class="description">
				<?php _e( 'You can use a site like <a href="http://www.getlatlon.com/" target="_blank">Get Lat Lon</a> to convert an address into coordinates.', 'risen' ); ?>
			</p>
		</div>
	</p>
	...
	class Risen_Events_Widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			<article class="events-widget-item<?php if ( 1 == $i ) : ?> events-widget-item-first<?php endif; ?>">
				...
				<header>
					<?php $event_type = get_post_meta( $post->ID , '_risen_event_type' , true ); ?>
					
					<h1 class="events-widget-item-title <?php echo $event_type; ?>"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h1>
					...
					if ( $venue ) :
					?>
					<div class="events-widget-item-venue"><?php echo $venue; ?></div>
					<?php endif; ?>
			...
			// No items found
			if ( empty( $i ) ) {
				_e( 'No upcoming events have been scheduled.', 'risen' );
			}
